...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-zinc-900 via-zinc-950 to-black font-sans text-zinc-100 flex flex-col md:flex-row overflow-x-hidden">
      ...
      <aside className="hidden md:flex w-64 glass-panel border-r border-white/10 shrink-0 z-20 sticky top-0 h-screen">
        <SidebarContent />
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 z-10 min-h-screen">
        {/* Glass Top Navbar */}
        <header className="h-16 border-b border-white/10 bg-zinc-950/70 backdrop-blur-xl px-4 md:px-8 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-3">
            {/* Mobile Hamburger Toggle Button */}
            <button
              onClick={() => setMobileMenuOpen(true)}
              className="p-2 rounded-xl bg-white/5 border border-white/10 text-zinc-300 hover:text-white hover:bg-white/10 md:hidden transition-all"
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            <span
              className={`inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-full text-[10px] sm:text-[11px] font-bold border shadow-sm ${
                isAdmin
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 w-full max-w-7xl mx-auto overflow-y-auto">{children}</main>
      </div>
    </div>
  );
}
