...
export default function TeacherReportsPage() {
  ...
  // Multi-Class Scope State: 'all' | 'single' | 'multi'
  ...
  // View Mode: Matrix Grid vs Summary Roster
  ...
  }, [classMode, selectedClassId, selectedClassIds, selectedSubjectId, startDate, endDate, user]);
  ...
  const fetchReportData = async () => {
    ...
    try {
      ...
      // Filter Students by Class Scope
      ...
      // 2. Fetch Sessions conducted in Date Range
      ...
      // Filter Sessions by Class Scope
      if (classMode === 'single' && selectedClassId) {
      ...
      // Filter Sessions by Subject Filter
  ...
  const handleExportGridCSV = () => {
    ...
    downloadCSVText(
      `Teacher_Grid_Report_${classTitle.replace(/\s+/g, '_')}_${subjectTitle.replace(/\s+/g, '_')}_${startDate}_to_${endDate}`,
      csvText
    );
  };
    ...
    <div className="space-y-6 text-xs">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2">
            Teacher Attendance Matrix Generator <FileSpreadsheet className="w-6 h-6 text-indigo-400" />
          </h1>
          <p className="text-xs text-zinc-400 mt-1">
            Generate 2-header-row attendance matrix reports for single or multiple combined classes within selected date ranges.
          </p>
        </div>
        ...
        <button
          ...
          className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-bold text-xs rounded-xl shadow-lg flex items-center gap-2 transition-all shrink-0"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 rounded-2xl bg-zinc-900/90 border border-zinc-800/80 backdrop-blur-sm shadow-md flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Enrolled Roster</span>
            <p className="text-2xl font-extrabold text-white mt-0.5">{students.length}</p>
            <p className="text-[10px] text-indigo-400 mt-0.5">Students in scope</p>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-zinc-900/90 border border-zinc-800/80 backdrop-blur-sm shadow-md flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Conducted Lectures</span>
            <p className="text-2xl font-extrabold text-white mt-0.5">{sessions.length}</p>
            <p className="text-[10px] text-emerald-400 mt-0.5">Sessions in date range</p>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-zinc-900/90 border border-zinc-800/80 backdrop-blur-sm shadow-md flex items-center justify-between">
          <div>
            ...
            <p className="text-sm font-bold text-purple-300 mt-1 truncate max-w-[150px]">
              {classMode === 'all'
                ? 'All Classes'
                : classMode === 'single'
            ...
            <p className="text-[10px] text-zinc-500 mt-0.5">Class combination</p>
          </div>
      ...
      {/* Control Panel: Multi-Class & Subject Filters + Date Range */}
      <div className="p-5 rounded-2xl bg-zinc-900/90 border border-zinc-800/80 shadow-xl space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-4 border-b border-zinc-800/80">
          ...
          <div className="space-y-2">
            ...
            {classMode === 'single' && (
              <select
                ...
                className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3.5 py-2 text-xs font-semibold text-white focus:outline-none focus:border-indigo-500"
              >
                {classes.map((c) => (
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3.5 py-2 text-xs font-semibold text-white focus:outline-none focus:border-emerald-500"
            >
        ...
        {/* Date Presets & View Switcher */}
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <Calendar className="w-4 h-4 text-amber-400" />
            <span className="text-xs font-bold text-zinc-300">Date Range Preset:</span>
            <div className="flex flex-wrap gap-1 bg-zinc-950 p-1 rounded-xl border border-zinc-800">
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold text-[11px] transition-all ${
                  datePreset === 'oneday' ? 'bg-indigo-600 text-white shadow' : 'text-zinc-400 hover:text-white'
                }`}
              >
                Today (1 Day)
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold text-[11px] transition-all ${
                  datePreset === 'weekly' ? 'bg-indigo-600 text-white shadow' : 'text-zinc-400 hover:text-white'
                }`}
              >
                Weekly (7 Days)
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold text-[11px] transition-all ${
                  datePreset === 'monthly' ? 'bg-indigo-600 text-white shadow' : 'text-zinc-400 hover:text-white'
                }`}
              >
                Monthly (30 Days)
              </button>
      ...
      {/* Matrix Table View (Matching Excel 2-Header Row Layout) */}
      <div className="rounded-2xl bg-zinc-900 border border-zinc-800 overflow-hidden shadow-xl">
        {loading ? (
          <div className="py-16 text-center text-xs text-zinc-500">Building teacher attendance matrix...</div>
        ) : filteredStudents.length === 0 ? (
          <div className="py-16 text-center text-xs text-zinc-500 border border-dashed border-zinc-800 rounded-xl m-6 p-8">
            <p className="text-zinc-300 font-bold mb-1">No student records match selected class or subject.</p>
            <p className="text-[11px] text-zinc-500">
              Try selecting a different class section or expanding the date preset.
            </p>
          </div>
        ) : viewMode === 'grid' ? (
          /* Attendance Matrix Grid View (Matching Image 2 Header Lines) */
