  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        ...
        const cleanPassword = formData.password.trim() || 'Password@123';
        let createdUserId = '';
        ...
        try {
          const authRes = await tempClient.auth.signUp({
            email: cleanEmail,
          ...
          if (authRes.data.user?.id) {
            createdUserId = authRes.data.user.id;
          }
        } catch (authErr: any) {
          console.warn('Auth registration notice:', authErr);
        }

        // Fallback ID if auth signup returned no user ID (e.g. user exists or auto-confirmed)
        if (!createdUserId) {
        ...
        // Upsert profile into public.users table with valid UUID
