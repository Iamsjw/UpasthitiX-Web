...
import { UserPlus, Search, X, UserCheck, GraduationCap, Trash2, Edit2, FileSpreadsheet, Upload, Key } from 'lucide-react';
...
export default function AdminUsersPage() {
    ...
    Papa.parse(csvFile, {
      ...
      complete: async (results) => {
        try {
          ...
          const { error } = await supabase.from('users').upsert(newUsers);
          if (error) {
  ...
  const openAddModal = (role: 'teacher' | 'student') => {
    setEditingUser(null);
    setFormData({ name: '', email: '', password: 'Password@123', role, roll_no: '', class_id: '' });
    setFormError('');
    setShowModal(true);
  };
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        // Update existing user profile in DB
        ...
        // Create User: Register Auth account & insert into public.users
        ...
        const { data: authData, error: authError } = await tempClient.auth.signUp({
          ...
          options: {
            data: { name: formData.name.trim(), role: formData.role },
          },
        });
        ...
        if (authData?.user?.id) {
          ...
          // Step 2: Handle existing registered Auth users by recovering ID
          if (
            authError.message.toLowerCase().includes('already') ||
            authError.message.toLowerCase().includes('exists')
          ) {
            try {
              const signInRes = await tempClient.auth.signInWithPassword({
                email: cleanEmail,
                password: cleanPassword,
              });
              if (signInRes.data?.user?.id) {
                createdUserId = signInRes.data.user.id;
              }
            } catch (_) {}

            if (!createdUserId) {
              try {
                const { data: rpcId } = await supabase.rpc('get_user_id_by_email', { user_email: cleanEmail });
                if (rpcId) createdUserId = rpcId;
              } catch (_) {}
            }
          }

          if (!createdUserId) {
            setFormError(`Auth Registration Failed: ${authError.message}`);
            setSubmitting(false);
            return;
          }
        }

        // Step 3: Insert profile into public.users with valid auth.users.id reference
              ...
              {!editingUser && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3.5 py-2.5 text-white focus:outline-none focus:border-indigo-500 font-mono text-xs"
                    required
                  />
                  <p className="text-[10px] text-zinc-500 mt-1">Default password set for new user login: Password@123</p>
                </div>
              )}
